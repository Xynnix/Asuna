...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.voice.channel) return [message.delete(), utils.timed_msg(utils.cmd_fail(`${message.author}, please join a voice channel to run this command!`, `${config.prefix}skip`), 5000)];
    ...
    if (!message.member.hasPermission('MANAGE_MESSAGES')) {
        ...
        message.channel.send(`🎵 ${message.author}, you have voted to skip! **${votes.votes}/3** votes`);
