...
module.exports.run = async (bot, message, args) => {
    ...
    if(premium[message.guild.id].premium === "False"){message.channel.send("This guild does not have premium status on asuna, To receve premium you must donate 2 euros per month")}
    else{
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (err) undefined
                    return message.channel.send(utils.cmd_fail('⚠ You have exceeded the 10 seconds selection time', `${config.prefix}search <music>`));
                }
